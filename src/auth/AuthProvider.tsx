...
type AuthContextValue = {
  ...
  updateRole:(role:UserRole)=>Promise<AuthResult>;
...
function readDemoUser(): User | null {
  if (supabase) return null; // Only read demo user if Supabase is not configured
  try {
...
fallback.verifiedPhone
),


verifiedEmail:Boolean(
profile.verified_email ??
fallback.verifiedEmail
),


verifiedBusiness:Boolean(
profile.verified_business ??
fallback.verifiedBusiness
),


successfulTransactions:Number(
profile.successful_transactions ??
fallback.successfulTransactions
),
...
let oauthRole:UserRole|undefined;
...
const profileUser=fromProfile(
updated.data.profile,
nextUser
);


saveDemoUser(profileUser);
