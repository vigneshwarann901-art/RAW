...
export default function ListRaw() {
  ...
  const scan = async (file?: File) => {
    ...
    setScanLoading(true);
    const result = await mockMaterialScan(file.name);
  ...
  const publish = () => {
    addListing({
      ...
      mode,
      image: image ?? '/material-copper.svg',
      trustScore: 94,
